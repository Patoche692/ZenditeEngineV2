...
std::vector<Texture2D> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
	...
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		std::string texStr = (str.C_Str());
		texStr = directory + "/" + texStr;

		bool skip = false;
		for (unsigned int j = 0; j < textures_loaded.size(); j++)
		{
			if (std::strcmp(textures_loaded[j].getTexFilePath().data(), texStr.c_str()) == 0)
			{
		...
		if (!skip)
		{
			Texture2D texture(typeName);
			
			texture.setupTexture(texStr.c_str());
