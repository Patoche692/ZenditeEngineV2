...
class RenderSpotLightSourceSystem : public I_System, public I_Subject
{
private:
...
public:
	RenderSpotLightSourceSystem(): m_LighSourceShader("res/shaders/LightingShaders/vs_LightSource.glsl", "res/shaders/LightingShaders/fs_LightSource.glsl")
	{
	...
	void Render(std::shared_ptr<I_Renderer> renderer, std::shared_ptr<I_API_Manager> apiManager, std::shared_ptr<ECSCoordinator> ECScoord)
	{
		...
		for (auto const& EID : m_EntitySet)
		{
			if (ECScoord->GetComponentDataFromEntity<c_LightRenderable>(EID).active == true)
			{
				renderer->RenderLightSource(apiManager->GetEntityDataHandle(EID), *ECScoord, EID, m_LighSourceShader, ECScoord->GetComponentDataFromEntity<c_SpotLightEmitter>(EID).diffuse);
			}
		}
	}
